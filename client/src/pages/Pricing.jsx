...
const handlePaying = async (amount) => {
    ...
    let addedCredits = 0;
    if (amount === 100) addedCredits = 50;
    else if (amount === 200) addedCredits = 120;
    else if (amount === 500) addedCredits = 300;

    // Simulate payment delay
    setTimeout(() => {
      const storedCredits = localStorage.getItem('prepgen_credits');
      let currentCredits = storedCredits !== null ? parseInt(storedCredits) : 100;
      currentCredits += addedCredits;
      localStorage.setItem('prepgen_credits', currentCredits);

      dispatch(updateCredits(currentCredits));
  ...
  return (
    <div className='min-h-screen bg-gray-100 px-6 py-10 relative'>
      ...
      <button onClick={()=>navigate("/")} className='flex items-center gap-2 cursor-pointer text-gray-600 hover:text-black mb-6'>
        ⬅️ Back
      </button>
